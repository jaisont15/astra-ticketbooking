		...
		if (action.equals("register")) {
			...
			if (user_name.equals("") || !Pattern.matches("^[\\w]+$", user_name)) {
				...
				if (!duplicate_name) {
					...
					String send = "Registration Complete" + newuser.getACCOUNTNAME();
		...
		// not logged in, go login
			...
			if (action.equals("updateprofile")) {
				...
				if (card_name == "" || card_num == ""
					...
					String seats = "Seat(s) are";
					String temp = "";
					int lastSeat = Integer.valueOf(order.getID())-201;
					for (int i = 0; i < ticket; i++) {
						temp = temp + " "+ lastSeat++;
					}
			seats = seats +temp;
					String send = "Your order number is:" + order.getID() + " and your seat numbers are "+temp+ '\n';
					mailManager.SendMail(addr, send);
				}
